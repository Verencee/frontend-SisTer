...
export default function CharacterModal({ character, onClose, onSuccess }: Props) {
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (character) {
        await dataAPI.put(`/characters/${character.id}`, payload);
      } else {
        await dataAPI.post("/characters", payload);
      }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white/10 backdrop-blur-xl border border-white/20 text-white rounded-xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <input name="image" defaultValue={character?.image ?? ""} placeholder="Image URL" className="w-full p-3 rounded-lg bg-white/10 border border-white/20 text-white" />
